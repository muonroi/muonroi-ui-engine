...
const M_ISSUE_ICON_ERROR = (
  <svg width={14} height={14} viewBox="0 0 16 16" fill="var(--mu-color-error)" style={{ flexShrink: 0, marginTop: 1 }}>
    <path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm2.97 4.03a.75.75 0 0 1 0 1.06L9.06 8l1.91 1.91a.75.75 0 1 1-1.06 1.06L8 9.06l-1.91 1.91a.75.75 0 1 1-1.06-1.06L6.94 8 5.03 6.09a.75.75 0 0 1 1.06-1.06L8 6.94l1.91-1.91a.75.75 0 0 1 1.06 0z"/>
  </svg>
);

const M_ISSUE_ICON_WARNING = (
  <svg width={14} height={14} viewBox="0 0 16 16" fill="var(--mu-color-warning)" style={{ flexShrink: 0, marginTop: 1 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {issues.map((issue) => {
        ...
        return (
          <div
            key={`${issue.code}:${issue.message}:${issue.fieldPath ?? ""}`}
            style={{
              display: "flex",
              ...
              borderRadius: 8,
              borderLeft: isError
                ? "3px solid var(--mu-color-error)"
                : "3px solid var(--mu-color-warning)",
              background: isError
                ? "var(--mu-color-error-bg)"
                : "var(--mu-color-warning-bg)",
              fontSize: "var(--mu-text-sm)",
            }}
          >
            {isError ? M_ISSUE_ICON_ERROR : M_ISSUE_ICON_WARNING}
            <span style={{ color: isError ? "var(--mu-color-error-text)" : "var(--mu-color-warning-text)", lineHeight: "20px" }}>
              <strong>{issue.code}</strong>{" "}{issue.message}
              {issue.fieldPath ? <span style={{ opacity: 0.7, marginLeft: 4 }}>({issue.fieldPath})</span> : null}
            </span>
          </div>
        );
      })}
